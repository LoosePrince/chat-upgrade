...
- **叠加式设置**：聊天面板左上角提供高 DPI 设置入口，可实时预览面板、输入栏、头像、双行布局、气泡、消息分栏、非玩家消息位置和右键菜单密度；保存提交整份配置，取消或关闭恢复打开时状态。
...
常用配置包括：聊天模式、外观与面板几何、上传路由、手动触发加载、平滑滚动、音频/视频音量、接收/上传上限等。聊天面板左上角设置弹窗覆盖全部客户端字段；面板几何也继续支持直接拖拽和缩放。
