...
/**
 * Minimal Stonecutter-style conditional-compilation preprocessor.
 *
 * Directives (line comments):
 * - `//? if >=26.2 {`            block start, active when the predicate matches the target version
 * - `//? } elif <26.1 {`          else-if branch
 * - `//? } else {`               else branch
 * - `//? }`                       block end
 * - `//? if >=26.2`               single-line form, applies only to the immediately following line
 *
 * Inside an active branch, content wrapped in `/* ... */` is uncommented so that the
 * "inactive default" branch can ship as a comment and be enabled for matching versions.
 */
object Preprocessor {
        ...
        for (line in lines) {
            ...
            if (directive != null) {
                when {
                    ...
                    directive == "} else {" -> {
                        val block = blocks.last()
                        block.active = block.parentActive && !block.matched
                        block.matched = true
                    }
                    ...
                    directive.startsWith("if ") && directive.endsWith(" {") -> {
                        ...
                        blocks.addLast(Block(currentActive(), active, active))
                    }
    ...
    private fun uncommentLine(line: String): String {
        // Only unwrap preprocessor-inactive lines like `/* code(); */`, not Javadoc (`/**`).
        ...
        if (open >= 0) result = result.removeRange(open, open + 2)
        val close = result.lastIndexOf("*/")
        if (close >= 0) result = result.removeRange(close, close + 2)
        return result
    }
    ...
    private fun evalVersion(version: String, condition: String): Boolean {
        ...
        if (parts.size == 1) {
            val match = Regex("(>=|<=|>|<|==)(.+)").matchEntire(parts[0])
                ?: error("Unsupported preprocessor condition: $condition")
        ...
        return when (operator) {
            ...
            "<" -> comparison < 0
            "==" -> comparison == 0
            else -> error("Unsupported preprocessor operator: $operator")
        }
    }
