...
public final class ServerChatRouteService {
    ...
    private static int routeFor(ServerPlayer target) {
        if (ServerPlayNetworking.canSend(target, ServerMediaPayloads.S2CStructuredChatMessage.TYPE)) {
            return ROUTE_STRUCTURED_MESSAGE;
        }
        if (ServerPlayNetworking.canSend(target, ServerMediaPayloads.S2CStructuredChatAttachment.TYPE)) {
    ...
    private static boolean sendStructuredMessage(ServerPlayer target, StructuredChatMessage message) {
        ServerPlayNetworking.send(target, ServerMediaPayloads.S2CStructuredChatMessage.fromMessage(message));
        return true;
    }
